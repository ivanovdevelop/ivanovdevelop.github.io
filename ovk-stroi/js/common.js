$(function() {
	...
	$(".carusel-eqipment, .carusel-brends").owlCarousel( { 
		loop:true,
		responsive:{
        0:{
            items:1,
...
$("a[href='#callback']").click(function() {
	...
	$(".form-callback [name=admin-data]").text(dataForm);
})

function heightses() {
		$(".s-direct .item-vertical p").height('auto').equalHeights();
		$(".carusel-text").height('auto').equalHeights();
		$(".testimonions-head").height('auto').equalHeights();
		$(".testimonions-text").height('auto').equalHeights();
	}
...
this.destroy();//разрушение то есть для показа только один раз
 }, {offset:'70%'});
...
$("body").on("click",".top",function() {
	...
	//код для перевода картинки в свг а не в имж для работы с  ксс
